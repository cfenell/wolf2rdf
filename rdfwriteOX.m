function rdfwriteOX(outpath,ooutgram,xoutgram,rdfpar,metadata)
...
%Path relative to work dir (htdocs)
...
%Check size of ionogram matrices
if size(ooutgram,1)~=150
  error('Wrong resizing: RDF file should always contain 150 heights!!')
  fclose(fid)
end

if size(xoutgram,1)~=150
  error('Wrong resizing: RDF file should always contain 150 heights!!')
  fclose(fid)
end

if size(ooutgram,2)~=size(xoutgram,2)
    error('Wrong resizing: Not the same number of frequencies in X and O mode ionograms!!')
  fclose(fid)
end

%Cast as 8-bit integer 0-255
ooutgram=int8(round(ooutgram));
xoutgram=int8(round(xoutgram));

%Write data to RDF file
for k=1:size(ooutgram,2) %loop over frequencies
  fwrite(fid,ooutgram(:,k),'integer*1'); %O mode 1-byte values for 150 altitudes
  fwrite(fid,xoutgram(:,k),'integer*1'); %X mode 1-byte values for 150 altitudes
end
